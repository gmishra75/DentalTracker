...
namespace NHDG.ProphySplitter {
	/// <summary>Represents a splitting rule with hard-coded fees.</summary>
	public class HardCodeSplitter : Splitter {
		/// <summary>Initializes an instance of HardCodeSplitter.</summary>
		...
		public HardCodeSplitter(int splitRuleID, string procedureCode, string carrier) : base(procedureCode, carrier) {
			...
			foreach (Treatment t in c.TreatmentInformation.Treatments) {
				if (t.ProcedureCode == this.ProcedureCode) {
					foreach (SplitRuleDetail d in this.ruleDetails) {
						...
						tempTreatment.Fee = Utilities.FormatCurrencyForXML(d.Value);
